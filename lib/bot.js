...
import { pwtokey, encrypt, decrypt } from './pbcrypt';
...
const encryptionHeader = '-----ENCRYPTED MESSAGE-----';
...
class Bot {
  constructor(options) {
    ...
    // Map of discord channels to secret list
  ...
  sendToIRC(message) {
    ...
    if (ircChannel) {
      ...
      message.content = this.decryptWrapper(message.channel.name, message.content);
  ...
  sendToDiscord(author, channel, text) {
    ...
    if (this.isCommandMessage(text)) {
      ...
      if (this.formatCommandPrelude) {
        const prelude = Bot.substitutePattern(this.formatCommandPrelude, patternMap);
        discordChannel.send(this.encryptWrapper(discordChannel, prelude));
      }
      discordChannel.send(this.encryptWrapper(discordChannel, text));
      return;
    }
    ...
    discordChannel.send(this.encryptWrapper(discordChannel, withAuthor));
  }
  ...
  /* Performs encryption to text passed if specified in configuration, otherwise returns text as is */
  encryptWrapper(channel, text) {
    if (this.encryption && this.encryption[channel.name] && this.encryption[channel.name].length) {
      // Perform encryption with first key
      return `${encryptionHeader}\n${encrypt(pwtokey(this.encryption[channel.name][0]), text.replace(/\</g, '&lt;').replace(/\>/g, '&gt;'))}`;
    } else {
      return text;
    }
  }

  /* Attempts decryption for an encrypted message coming from a channel configured from encryption */
  decryptWrapper(channel, text) {
    text = text.replace(/\[sec\]/g, '&#91;sec&#93;').replace(/\[secfail\]/g, '&#91;secfail&#93;');
    if (this.encryption && this.encryption[channel] && this.encryption[channel].length && text.startsWith(encryptionHeader)) {
      // Attempt decryption
      var result = `[secfail] Could not decrypt. Sender is using a bad secret, or a new secret that the bot does not know of yet.\n${text}`;
        for (var secret of this.encryption[channel]) {
          try {
            result = `[sec] ${decrypt(pwtokey(secret), text.replace(encryptionHeader, '').trim())}`;
            return result;
